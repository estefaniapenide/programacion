...
//atributos
    ...
    private static LinkedList<AsignaVacuna> listaVacunasAsignadas = new LinkedList<AsignaVacuna>();
...
//métodos
     ...
     *
     * @param codigo clave(String) código de la vacuna
     * @param vacuna valor(Vacuna) vacuna que se desea añadir a la lista
     */
     ...
     * vacío.
     *
     * @param nombre (String) nombre de la vacuna
     * @return resultadoBusqueda
     */
     ...
     * vacío.
     *
     * @param principioActivo (String) principio activo de la vacuna
     * @return resultadoBusqueda
     */
     ...
     * vacío.
     *
     * @param farmaceutica (String) famaceutica de la vacuna
     * @return resultadoBusqueda
     */
     ...
     * vacío.
     *
     * @param precioRecomendado (double) precio recomendado de la vacuna
     * @return resultadoBusqueda
     */
...
//método para ver la última fase implementada de cada vacuna
    ...
    public static LinkedList<AsignaVacuna> agregarVacunasAsignadas(AsignaVacuna nuevaAsignacion) {

        listaVacunasAsignadas.addLast(nuevaAsignacion);
        return listaVacunasAsignadas;
    }

    public static LinkedList<AsignaVacuna> verVacunasAsignadasPorPais(String pais) {

        LinkedList<AsignaVacuna> vacunasAsignadasPorPais = new LinkedList<AsignaVacuna>();
        for (int i = 0; i < listaVacunasAsignadas.size(); i++) {
            if (listaVacunasAsignadas.get(i).getPais().equals(pais)) {
                vacunasAsignadasPorPais.addLast(listaVacunasAsignadas.get(i));
            }
        }

        return vacunasAsignadasPorPais;
    }

    public static boolean existeVacunasAsignadas(AsignaVacuna buscarAsignacion) {

        boolean existe = false;

        for (int i = 0; i < listaVacunasAsignadas.size(); i++) {
            if (listaVacunasAsignadas.get(i).equals(buscarAsignacion)) {
                existe = true;
            }
        }
        return existe;
    ...
    public static AsignaVacuna buscarVacunaAsignada(AsignaVacuna buscarAsignacion) {

        AsignaVacuna resultadoBusqueda = new AsignaVacuna(null, "");
        for (int i = 0; i < listaVacunasAsignadas.size(); i++) {
            if (listaVacunasAsignadas.get(i).equals(buscarAsignacion)) {
                resultadoBusqueda = listaVacunasAsignadas.get(i);
            }
        }
        return resultadoBusqueda;
    }
